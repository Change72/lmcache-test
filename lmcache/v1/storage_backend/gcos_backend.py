...
class GcosBackend(StorageBackendInterface):
    """
    GCOS Backend that leverages direct GPU-to-storage operations using GCOS
    file system for high-performance KV cache storage. This backend provides
    zero-copy operations and direct GPU access to persistent storage.

    Cache Directory Structure created by this Backend:
    /{gcos_path}/{first_level}/{second_level}/{data & metadata}
    """
    ...
    def __init__(
        ...
        assert config.gcos_path is not None, "Need to specify gcos_path for GcosBackend"
        self.gcos_path = config.gcos_path
        
        logger.info(f"GCOS backend using path '{self.gcos_path}'")

        # Initialize GCOS system
        self.gcos_initialized = False
        try:
            # Import GCOS operations
            import lmcache.gcos_ops as gcos_ops
            self.gcos_ops = gcos_ops
            
            # Initialize GCOS with config
            gcos_config_path = config.extra_config.get("gcos_config", "gcos.json")
            self.gcos_ops.initialize_gcos(gcos_config_path)
            self.gcos_ops.init_allocator()
            self.gcos_initialized = True
            logger.info("GCOS system initialized successfully")
            
        except ImportError:
            logger.warning("GCOS not available, falling back to standard file operations")
            self.gcos_ops = None
        except Exception as e:
            logger.error(f"Failed to initialize GCOS: {e}")
            self.gcos_ops = None

        if not os.path.exists(self.gcos_path):
            os.makedirs(self.gcos_path, exist_ok=True)
        ...
        # GCOS uses direct memory mapping
        if hasattr(self.memory_allocator, "base_pointer"):
            logger.debug(f"Using base pointer {self.memory_allocator.base_pointer}")
            self.gcos_base_pointer = self.memory_allocator.base_pointer
        else:
            logger.info("No base pointer found, GCOS will use fallback operations")
            self.gcos_base_pointer = None
    ...
    async def _scan_metadata(self):
        # Scan existing cache metadata on startup
        tasks = []
        start = time.perf_counter()
        with os.scandir(self.gcos_path) as it:
            for entry in it:
                if not entry.is_dir():
                    continue
                ...
                tasks.append(
                    asyncio.to_thread(
                        ...
                        os.path.join(self.gcos_path, l1_dir),
                        l1_dir,
                    )
                )
    ...
    async def _async_save_bytes_to_disk(
        ...
        """
        Convert KV to bytes and async store bytes to disk using GCOS.
        """
        ...
        if subdir_key not in self.metadata_dirs:
            os.makedirs(os.path.join(self.gcos_path, l1_dir, l2_dir), exist_ok=True)
            self.metadata_dirs.add(subdir_key)
    ...
    def _load_bytes_from_disk(
        ...
        """
        Load byte array from disk using GCOS direct operations.
        """
        ...
        if self.gcos_base_pointer is None:
            addr = ctypes.c_void_p(memory_obj.tensor.data_ptr())
            dev_offset = 0
        else:
            addr = ctypes.c_void_p(self.gcos_base_pointer)
            dev_offset = memory_obj.metadata.address
        
        ret = self._load_gcos(path, offset, addr, memory_obj.get_size(), dev_offset)
    ...
    def _save_gcos(
        ...
        try:
            ...
            # Use GCOS direct GPU-to-storage if available
            if self.gcos_ops and self.gcos_initialized:
                # Create GCOS file and DMA buffer
                filename = os.path.basename(tmp_path)
                gcos_file = self.gcos_ops.create_gcos_file(filename, kv_chunk.nbytes + _METADATA_MAX_SIZE)
                ...
                # Create DMA buffer for the tensor
                dma_buf = self.gcos_ops.create_gcos_dma_buffer(kv_chunk.nbytes)
                
                # Write to GCOS file
                self.gcos_ops.gpu_file_write(gcos_file, dma_buf, kv_chunk.nbytes, 0)
                
            else:
                # Fallback to standard file operations
                logger.debug("Using fallback file operations for GCOS backend")
                with open(tmp_path, "r+b") as f:
                    f.seek(_METADATA_MAX_SIZE)
                    # Copy GPU tensor to CPU then to file
                    cpu_tensor = kv_chunk.cpu()
    ...
    def _load_gcos(
        ...
        """
        Load data from GCOS storage directly to GPU memory.
        """
        try:
            if self.gcos_ops and self.gcos_initialized:
                # Use GCOS direct operations
                filename = os.path.basename(gcos_path)
                gcos_file = self.gcos_ops.create_gcos_file(filename, 0)  # Open existing
                dma_buf = self.gcos_ops.create_gcos_dma_buffer(size_in_bytes)
                
                # Direct GPU read
                bytes_read = self.gcos_ops.gpu_file_read(gcos_file, dma_buf, size_in_bytes, 0)
                return bytes_read
            else:
                # Fallback to standard operations
                with open(gcos_path, "rb") as f:
                    f.seek(file_offset)
                    data = f.read(size_in_bytes)
                    if len(data) != size_in_bytes:
                        return len(data)
                    
                    # Copy data to GPU memory
                    cpu_array = torch.frombuffer(data, dtype=torch.uint8)
                    gpu_tensor = cpu_array.cuda()
                    
                    # Copy to target GPU memory location
                    target_ptr = ctypes.c_void_p(int(gpu_pointer.value) + dev_offset)
                    ctypes.memmove(target_ptr, gpu_tensor.data_ptr(), size_in_bytes)
                    
                return size_in_bytes
                
        except Exception as e:
            logger.error(f"Error loading from {gcos_path}: {e}")
            return -1
    ...
    def close(self) -> None:
        if self.gcos_ops and self.gcos_initialized:
            try:
                self.gcos_ops.destroy_allocator()
                self.gcos_ops.cleanup_gcos()
                logger.info("GCOS system cleaned up")
            except Exception as e:
                logger.error(f"Error during GCOS cleanup: {e}")
        logger.info("GCOS backend closed.")