...
from lmcache.v1.storage_backend.gcos_backend import GcosBackend
...
def CreateStorageBackends(
    ...
    if config.gcos_path is not None:
        gcos_backend = GcosBackend(config, loop, memory_allocator, dst_device)
        storage_backends[str(gcos_backend)] = gcos_backend
